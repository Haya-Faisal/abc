...
//get = provide whatv theb server is asking
//post= tell the server what to do
...
app.post("/upload-photo", (req, res) => {
  ...
  req.on("end", () => {
    ...
    let daatastext = JSON.stringify(photos, null, 2);
    fs.writeFileSync("photos.json", daatastext, "utf8");
    // let online users know about the new path
    io.emit("new-photo", imgdata);
  });
});
...
io.on("connection", (socket) => {
  ...
  io.emit("historic-photos", imgdata);
