...
function initSocket() {
  ...
  socket.on("start-snake", () => {
    ...
    document.getElementById("one-phone").style.display = "none";
  ...
  socket.on("your-role", (role) => {
    ...
    //when the buttom is pressed and only one phone is there
    document.getElementById("waiting-message").style.display = "none";
    document.getElementById("one-phone").style.display = "block";
  });

  socket.on("all-here", () => {
    state = "waiting";
    //all hv joined. start agem
    document.getElementById("one-phone").style.display = "none";
    // document.getElementById("waiting-message").style.display = "none";
  });
      ...
      setTimeout(() => {
        ...
        if (hassnake) {
          instructions.innerText =
            " The snake is controlled by the screen it is not displayed on!";
        } else {
          instructions.innerText =
            "The snake is controlled by the screen it is not displayed on!";
        }
...
function updateLevel2Instructions() {
  ...
  if (hassnake) {
    ...
    instructions.innerText =
      "Tilt your phone to control the snake on the other screen!";
  }
}
